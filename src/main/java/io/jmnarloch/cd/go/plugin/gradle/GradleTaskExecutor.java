...
public class GradleTaskExecutor implements TaskExecutor {
    ...
    private static ProcessBuilder buildGradleProcess(ExecutionConfiguration config, ExecutionContext environment) {
        ...
        workingDirectory = StringUtils.isBlank(relativePath)?workingDirectory : unifyPath(workingDirectory+relativePath);        final List<String> command = parse(config, env, workingDirectory);
